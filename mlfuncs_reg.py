...
def fn_reg_performance(y, y_pred):
    ...
    mape = (abs(y-y_pred)/abs(y)).mean()
    r2 = 1 - (y - y_pred).std()**2/y.std()**2 

    return np.array([rmse, mae, mape, r2]).round(3)
...
def fn_reg_metrics_tr_eval(y_tr, y_tr_pred, y_eval, y_eval_pred):
    ...
    absolute_error, percent_error = abs(y_tr - y_tr_pred), abs(y_tr - y_tr_pred)/abs(y_tr)

    rmse_2, mae_2, mape_2, r2_2 = np.array(fn_reg_performance(y_eval, y_eval_pred)).round(3)
    absolute_error_2, percent_error_2 = abs(y_eval - y_eval_pred), abs(y_eval - y_eval_pred)/abs(y_eval)
